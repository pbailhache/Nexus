...
var initDATPLANET = function(other_planets,context)
{
	for (var i = other_planets.length - 1; i >= 0; i--) 
	{
		if (other_planets[i].owner.id != 5)
		{
			DAT_PLANET = other_planets[i];
			break;
		}
	};
}
...
var getOrders = function(context) 
{
	...
	initDATPLANET(other_planets,context);
	setMessage(DAT_PLANET);
